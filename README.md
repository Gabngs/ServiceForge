...
> **Estado: v2.3.1 — prototipo.** Segunda versión estable de la herramienta en modo prototipo: el flujo completo funciona de punta a punta, pero todavía se usa y se valida contra proyectos reales antes de considerarla de producción.

## Qué hace

- Conecta a MySQL/MariaDB y analiza la tabla elegida (`DESCRIBE`, índices únicos, tablas del schema). Botón de "Probar conexión" separado de "Conectar", con indicador de estado (conectando / conectado / error) sin congelar la ventana. Los selectores de tablas (la tabla a analizar y el "FK -> tabla" de cada columna) se abren con todas las opciones y se pueden escribir para filtrar la lista, sin recorrerla con el scroll.
...
- Detección de Model/Resource ya existentes en el proyecto, asistida por un modelo entrenado para reconocer el patrón (Model → Filter → Requests → Resource → Service → Controller → ruta) aunque el proyecto use otras carpetas, nombres o prefijos. La red propone y el desarrollador decide; sigue aprendiendo de cada confirmación.
- **Estructura del proyecto:** al analizar la primera tabla, un diálogo compara la estructura del estándar con la que el proyecto ya usa (carpetas y nombres de cada tipo de archivo) y deja elegir cuál usar, o escribir otra, por tipo de archivo; "Auto-registrar" aplica lo detectado en un paso. Los archivos se generan dentro de la estructura elegida, con sus namespaces e imports.
- **Relaciones confirmadas antes de generar:** para cada FK, un diálogo propone qué Model y qué RelationResource usa el código (con la confianza de la red), en vez de suponerlo por el nombre de la tabla. Se puede elegir otro archivo del proyecto, generar uno nuevo o dejar la columna plana.
- **Mapa del proyecto** (menú Mapa): lo confirmado se guarda en notas `.md` con `[[wikilinks]]` (abribles en Obsidian) y vale para cualquier módulo que use esa tabla. Se exporta e importa para llevarlo a otra PC.
